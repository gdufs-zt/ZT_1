...
public class CartServiceImpl implements CartService {
    ...
    public List<CartVO> list(Integer userId) {
        List<CartVO> cartVOS = cartMapper.selectList(userId);    //调用cartMapper来查找特定用户的购物车记录
        for (int i = 0; i < cartVOS.size(); i++) {              //对于每一条购物车记录计算其总价
    ...
    public List<CartVO> add(Integer userId, Integer productId, Integer count) {
        validProduct(productId, count);     //检查商品是否存在、商品是否可售、库存检查

        Cart cart = cartMapper.selectCartByUserIdAndProductId(userId, productId);    //查找该用户的购物车是否有该商品
        if (cart == null) {
    ...
    public List<CartVO> delete(Integer userId, Integer productId) {
        ...
        if (cart == null) {
            //这个商品之前不在购物车里，无法删除
            throw new dsMallException(dsMallExceptionEnum.DELETE_FAILED);
        } else {
            //这个商品已经在购物车里了，则可以删除
