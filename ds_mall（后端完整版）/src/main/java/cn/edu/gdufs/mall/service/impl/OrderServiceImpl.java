...
import cn.edu.gdufs.mall.common.Constant.Cart;
import cn.edu.gdufs.mall.common.Constant.OrderStatusEnum;
import cn.edu.gdufs.mall.common.Constant.SaleStatus;
...
import cn.edu.gdufs.mall.model.dao.CartMapper;
import cn.edu.gdufs.mall.model.dao.OrderItemMapper;
import cn.edu.gdufs.mall.model.dao.OrderMapper;
import cn.edu.gdufs.mall.model.dao.ProductMapper;
import cn.edu.gdufs.mall.model.pojo.Order;
import cn.edu.gdufs.mall.model.pojo.OrderItem;
...
import cn.edu.gdufs.mall.model.vo.OrderItemVO;
...
import cn.edu.gdufs.mall.util.OrderCodeFactory;

import cn.edu.gdufs.mall.util.QRCodeGenerator;
...
public class OrderServiceImpl implements OrderService {
    ...
    public String create(CreateOrderReq createOrderReq) {
        ...
        if (CollectionUtils.isEmpty(cartVOList)) {
            throw new dsMallException(dsMallExceptionEnum.CART_EMPTY);
        }
        //判断商品是否存在、上下架状态、库存
        ...
        for (int i = 0; i < orderItemList.size(); i++) {
            ...
            if (stock < 0) {
                throw new dsMallException(dsMallExceptionEnum.NOT_ENOUGH);
            }
    ...
    private void validSaleStatusAndStock(List<CartVO> cartVOList) {
        for (int i = 0; i < cartVOList.size(); i++) {
            ...
            if (product == null || product.getStatus().equals(SaleStatus.NOT_SALE)) {
                throw new dsMallException(dsMallExceptionEnum.NOT_SALE);
            }
            //判断商品库存
    ...
    public OrderVO detail(String orderNo) {
        ...
        //订单不存在，则报错
        if (order == null) {
            throw new dsMallException(dsMallExceptionEnum.NO_ORDER);
        }
        //订单存在，需要判断所属
        Integer userId = UserFilter.currentUser.getId();
        if (!order.getUserId().equals(userId)) {
            throw new dsMallException(dsMallExceptionEnum.NOT_YOUR_ORDER);
        }
    ...
    public void cancel(String orderNo) {
        ...
        if (order.getOrderStatus().equals(OrderStatusEnum.NOT_PAID.getCode())) {
            ...
            throw new dsMallException(dsMallExceptionEnum.CANCEL_WRONG_ORDER_STATUS);
        }
        //恢复商品库存
    ...
    public void pay(String orderNo) {
        ...
        if (order.getOrderStatus() == OrderStatusEnum.NOT_PAID.getCode()) {
            ...
            throw new dsMallException(dsMallExceptionEnum.PAY_WRONG_ORDER_STATUS);
        }
    }
    ...
    public void deliver(String orderNo) {
        ...
        if (order.getOrderStatus() == OrderStatusEnum.PAID.getCode()) {
            ...
            throw new dsMallException(dsMallExceptionEnum.DELIVER_WRONG_ORDER_STATUS);
        }
    }
    ...
    public void finish(String orderNo) {
        ...
        if (order.getOrderStatus() == OrderStatusEnum.DELIVERED.getCode()) {
            ...
            throw new dsMallException(dsMallExceptionEnum.FINISH_WRONG_ORDER_STATUS);
        }
    }
}